  ...
  return (
    <ProtectedRoute>
      <div className="mx-auto max-w-4xl px-4 py-8 sm:py-12 animate-fade-in space-y-8">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight sm:text-3xl">Account Settings</h1>
          <p className="text-slate-600 text-sm mt-1">Manage your identity, avatar URL, and competitive programming profiles.</p>
        </div>
        ...
        <div className="grid gap-8 md:grid-cols-3">
            ...
            <Card className="border-slate-200 bg-white/40 backdrop-blur-sm">
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="flex flex-col gap-6 sm:flex-row sm:items-center border-b border-slate-200 pb-5">
                    <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-full border border-slate-200 bg-white shadow-inner">
                      {avatarUrl ? (
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 border-t border-slate-200 pt-5">
                    {/* LeetCode handle */}
                    <div className="space-y-2">
                      <Label htmlFor="leetcodeHandle" className="flex items-center gap-1.5 text-slate-600">
                        LeetCode Username
                      </Label>
                      <Input
                        id="leetcodeHandle"
                        ...
                        className="bg-slate-50 border-slate-200 focus-visible:ring-indigo-500 font-mono text-xs"
                      />
                    </div>

                    {/* Codeforces handle */}
                    <div className="space-y-2">
                      <Label htmlFor="codeforcesHandle" className="flex items-center gap-1.5 text-slate-600">
                        Codeforces Username
                      </Label>
                      <Input
                        id="codeforcesHandle"
          ...
          <div className="space-y-6">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Coding Profiles</h3>

            {/* LeetCode Profile */}
            {user?.leetcodeHandle ? (
              <Card className="border-slate-200 bg-white/20 backdrop-blur-sm overflow-hidden">
                <div className="bg-amber-500/10 p-3.5 border-b border-amber-500/10 flex items-center justify-between">
                  ...
                  <a
                    ...
                    className="text-[10px] flex items-center gap-1 text-slate-600 hover:text-slate-900"
                  >
                ...
                <CardContent className="p-4 space-y-2 font-sans">
                  ...
                  <p className="font-mono text-sm text-slate-900 font-bold">{user.leetcodeHandle}</p>
              ...
              <Card className="border-slate-200 bg-white/10 p-4 text-center">
                <p className="text-xs text-slate-500 italic">No LeetCode handle configured.</p>
              </Card>
            )}
            ...
            {user?.codeforcesHandle ? (
              <Card className="border-slate-200 bg-white/20 backdrop-blur-sm overflow-hidden">
                ...
                <CardContent className="p-4 space-y-3.5">
                  {loadingStats ? (
                    <div className="text-center py-4 text-xs text-slate-500">Loading CF stats...</div>
                  ) : cfStats ? (
                    <div className="flex items-center gap-3">
                      ...
                      <div className="space-y-0.5">
                        <p className="font-mono text-xs text-slate-600 font-bold">{cfStats.handle}</p>
                        <p className="text-xs text-slate-900 capitalize font-semibold">
                          Rank: <span className="text-blue-400">{cfStats.rank || "Unranked"}</span>
                        </p>
                        <div className="flex items-center gap-3 text-[10px] text-slate-500">
                          <span>Rating: <strong className="text-slate-900">{cfStats.rating || 0}</strong></span>
                          <span>Max: <strong className="text-slate-900">{cfStats.maxRating || 0}</strong></span>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="space-y-1">
                      <p className="text-xs font-semibold text-slate-700">Handle</p>
                      <p className="font-mono text-sm text-slate-900 font-bold">{user.codeforcesHandle}</p>
              ...
              <Card className="border-slate-200 bg-white/10 p-4 text-center">
                <p className="text-xs text-slate-500 italic">No Codeforces handle configured.</p>
              </Card>
            )}
          </div>
        </div>
